...
import * as usuarioService from '@/services/usuario/usuario.service'
import type { GetUsersParams } from '@/services/usuario/interfaces/get-users-params.interface'
  ...
  if (user.username && user.email) {
    try {
      if (dialogMode.value === 'new') {
        await usuarioService.registerAdmin(user)
        toast.add({ severity: 'success', summary: 'Éxito', detail: 'Usuario creado', life: 3000 })
      } else {
        await usuarioService.update(user.id, { username: user.username, email: user.email })
        toast.add({
          severity: 'success',
          summary: 'Éxito',
...
const deactivateUser = async (id: number) => {
  try {
    await usuarioService.changeEstado(id)
